...
static bool is_num_char(const char c) {
  return (isdigit(c) || (c == '.') || (c == '-'));
}

static void ignore_whitespace(parse_state &s) {
...
static int parse_int(parse_stream<string> &s) {
...
static addr parse_address(char c, parse_stream<string> &s) {
  switch (c) {
  case 'X':
  case 'Y':
  case 'Z':
...
string parse_line_comment_with_delimiter(string, parse_stream<string> &s) {
  string text = "";
  while (s.chars_left()) {
    text += s.next();
    s++;
...
void GCodeParser::output_intermediate(
            double target_x_pos,
            double target_y_pos,
            double target_z_pos,
            double feed_rate,
...
void GCodeParser::arc_xz(double position_x, double position_y, double position_z,
...
const std::vector<std::string> &GCodeParser::intermediate_code() const {
    return _intcode;
}

const std::vector<std::string> &GCodeParser::g_code() const {
    return _gcode;
}

void GCodeParser::loadAndParse(std::ifstream &t) {
